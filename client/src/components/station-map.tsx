...
export function StationMap({ onStationSelect }: StationMapProps) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full">
      {/* Map Header */}
      <div className="p-4 bg-radio-black border-b border-vdu-green-dim">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-vdu-green font-serif">Global Radio Map</h2>
            <p className="text-sm text-gray-400 mt-1">
              {validStations.length} stations with location data • Zoom and click to explore
            </p>
          </div>
          <div className="flex items-center space-x-2 text-xs text-gray-400">
            <Globe className="w-4 h-4" />
            <span>{stations.length} total stations loaded</span>
          </div>
        </div>
      </div>
